...
public class FirebaseChatService extends Service {
    ...
    public void onCreate() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_MAIN, "Instant chat service", NotificationManager.IMPORTANCE_LOW);
        ...
        localBroadcastManager.registerReceiver(locationReceiver, new IntentFilter(Helper.SEND_LOCATION));
    ...
    private final BroadcastReceiver uploadAndSendReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction() != null && intent.getAction().equals(Helper.UPLOAD_AND_SEND)) {
                Log.d(TAG, "onReceive: upload and send reached");
    ...
    private final BroadcastReceiver locationReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction() != null && intent.getAction().equals(Helper.SEND_LOCATION)) {
                Log.d(TAG, "onReceive: upload and send reached");
                Attachment attachment = intent.getParcelableExtra("attachment");
                int type = intent.getIntExtra("attachment_type", -1);
                String attachmentChatChild = intent.getStringExtra("attachment_chat_child");
                ...
                Group attachmentRecipientGroup = intent.getParcelableExtra("attachment_recipient_group");
//                uploadAndSend(new File(attachmentFilePath), attachment, type, attachmentChatChild, attachmentRecipientId, attachmentRecipientUser, attachmentRecipientGroup);
                sendMessage("Check this Address!", type, attachment, attachmentChatChild, attachmentRecipientId, attachmentRecipientUser, attachmentRecipientGroup);
            }
        }
    };
    ...
    private void uploadAndSend(final File fileToUpload,
        ...
        Log.d(TAG, "uploadAndSend: reached");
        if (!fileToUpload.exists()) {
            Log.d(TAG, "uploadAndSend: no file found");
            return;
        }
        ...
        Log.d(TAG, "uploadAndSend: starting upload" + fileName);
        storageReference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            ...
            public void onSuccess(Uri uri) {
                ...
                Log.d(TAG, "onSuccess: file uploaded: " + uri.toString());
            ...
            public void onFailure(@NonNull Exception exception) {
                ...
                FirebaseUploader firebaseUploader = new FirebaseUploader(new FirebaseUploader.UploadListener() {
                    ...
                    public void onUploadSuccess(String downloadUrl) {
                        ...
                        attachment1.setBytesCount(fileToUpload.length());
                        Log.d(TAG, "onUploadSuccess: file uploaded: " + downloadUrl);
                        sendMessage(null, attachmentType, attachment1, chatChild, recipientId, recipientUser, recipientGroup);
                    }
                    ...
                    public void onUploadCancelled() {
                        Log.d(TAG, "onUploadCancelled: some thing f");
                    }
                }, storageReference);
    ...
    private final ChildEventListener chatUpdateListener = new ChildEventListener() {
        @Override
